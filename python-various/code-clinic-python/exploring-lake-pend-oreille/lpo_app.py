...
class LPOApp:
    ...
    def _create_gui(self):
        ...
        self.style.configure("TButton", background=bg_color, font=("Arial Black", 10))
        self.style.configure("TLabel", background=bg_color, font=("Arial Black", 10))
        self.style.configure("Status.TLabel", background=bg_color, font=("Arial", 10))
        ...
        ttk.Label(self.frame_input, text="Start Date:").grid(
            row=0, column=1, columnspan=3, sticky="sw"
        )
        ttk.Label(self.frame_input, text="End Date:").grid(
            row=0, column=5, columnspan=3, sticky="sw"
        )
        ...
        ttk.Button(self.frame_input, text="Submit", command=self._submit_callback).grid(
            row=2, column=0, columnspan=9, pady=5
        )
        ...
        ttk.Label(
            self.frame_result, textvariable=self.air_temp_mean, style="Result.TLabel"
        ).grid(row=1, column=2)
        ttk.Label(
            self.frame_result, textvariable=self.air_temp_median, style="Result.TLabel"
        ).grid(row=2, column=2)
        ttk.Label(
            self.frame_result,
            textvariable=self.barometric_press_mean,
            style="Result.TLabel",
        ).grid(row=1, column=3)
        ttk.Label(
            self.frame_result,
            textvariable=self.barometric_press_median,
            style="Result.TLabel",
        ).grid(row=2, column=3)
        ttk.Label(
            self.frame_result, textvariable=self.wind_speed_mean, style="Result.TLabel"
        ).grid(row=1, column=1)
        ttk.Label(
            self.frame_result, textvariable=self.wind_speed_mean, style="Result.TLabel"
        ).grid(row=2, column=1)
    ...
    def _submit_callback(self):
        ...
        if data != []:
            ...
            # calculate the mean & median for each type of data; store result in
            # dictionaries
            result = {}
            for key in dict_of_lists.keys():
                result[key] = dict(
                    mean=mean(dict_of_lists[key]), median=median(dict_of_lists[key])
                )
            ...
            self.barometric_press_mean.set(
                "{0:.2f}".format(result["Barometric_Press"]["mean"])
            )
            self.barometric_press_median.set(
                "{0:.2f}".format(result["Barometric_Press"]["median"])
            )
