...
class Theremin:
    ...
    def mouse_move(self, event):
        ...
        R = int(
            ...
            * (self.canvas.winfo_height() - (event.y + 1))
        ...
        self.canvas.config(
            background="#{:02x}50{:02x}".format(R, B)
        )  # set green to 0x50 for "pizzazz"
    ...
    def calc_freq(self, y_pos):  # calculate frequency of "piano key" at y_pos
        pkey = (
            88 * (self.canvas.winfo_height() - (y_pos + 1)) / self.canvas.winfo_height()
            + 1
        )
    ...
    def sine_wave(self, frequency=440.0, samplerate=22050):
        ...
        return (32767 * np.sin(np.arange(int(samples)) * (2 * np.pi) / samples)).astype(
            np.int16
        )
