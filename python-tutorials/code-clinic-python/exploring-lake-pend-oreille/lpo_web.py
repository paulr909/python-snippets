...
def get_data_for_date(date):
    """
    Returns a generator object of data for the specified date.
...
def _get_data_post2006(date):
    ...
    for key in data.keys():
        try:
            data[key] = (
                request.urlopen("{}{}".format(url, key))
                .read()
    ...
    for i in range(len(data["Air_Temp"])):
        ...
        yield dict(
            Date=data["Air_Temp"][i].split()[0],
            Time=data["Air_Temp"][i].split()[1],
            Status="PARTIAL"
            if date == date.today()
            else "COMPLETE",  # assume data from today is incomplete
