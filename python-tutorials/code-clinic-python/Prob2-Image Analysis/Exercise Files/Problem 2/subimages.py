...
class FindSubset:
    def __init__(self, master):
        ...
        self.browse_button = ttk.Button(
            self.main_frame, text="Browse...", command=self.browse_callback
        )
        self.browse_button.grid(row=1, column=1, sticky="w")

        self.search_button = ttk.Button(
            self.main_frame, text="Find Subset Images", command=self.search_callback
        )
        ...
        self.progressbar = ttk.Progressbar(
            self.status_frame, mode="determinate", variable=self.progress_var
        )
    ...
    def search_callback(self):
        ...
        try:  # build list of all jpg image files in directory
            ...
            images = list(
                entry for entry in os.listdir(self.path) if entry.endswith(".jpg")
            )
        except:
        ...
        if len(images) < 2:
            messagebox.showerror(
                title="Not Enough Images", message="Need at least 2 images to analyze."
            )
            return
        ...
        self.master.after(
            10, self.process_queue
        )  # provides time for tkinter to update GUI
    ...
    def process_queue(self):
        ...
        if (temp_img.size[0] < orig_img.size[0]) and (
            temp_img.size[1] < orig_img.size[1]
        ):
            ...
            if (orig_img.size[0] > MAX_WIDTH) or (orig_img.size[1] > MAX_HEIGHT):
                ...
                ratio = min(
                    MAX_WIDTH / float(orig_img.size[0]),
                ...
                temp_img = temp_img.resize(
                    (int(ratio * temp_img.size[0]), int(ratio * temp_img.size[1])),
                    Image.ANTIALIAS,
                )
            else:
            ...
            if (
                ratio != 1
            ):  # if images were resized, get originals and calculate corresponding match_loc
                match_loc = (int(match_loc[0] / ratio), int(match_loc[1] / ratio))
                orig_img = Image.open(os.path.join(self.path, pair[0]))
                temp_img = Image.open(os.path.join(self.path, pair[1]))

            # get matching subsection from original image (using RGB mode)
            orig_sub_arr = np.array(orig_img)[
                match_loc[0] : match_loc[0] + temp_img.size[0],
            ...
            sum_of_squares = sum(
                value * ((idx % 256) ** 2) for idx, value in enumerate(h_diff)
            )
            ...
            if RMS_THRESHOLD > rms:  # add matches to table
                ...
                self.results_table.insert(
                    "", "end", str(self.progress_var.get()), text=pair[0]
                )
                self.results_table.set(str(self.progress_var.get()), "subset", pair[1])
                self.results_table.config(
                    height=len(self.results_table.get_children(""))
                )

        self.progressbar.step()
        self.status_var.set(
            "Analyzed {} vs {} - {} pairs remaining...".format(
                pair[0], pair[1], self.queue.qsize()
            )
        )
        ...
        if not self.queue.empty():
            ...
            self.status_var.set(
                "Done - Elapsed Time: {0:.2f} seconds".format(elapsed_time)
            )
