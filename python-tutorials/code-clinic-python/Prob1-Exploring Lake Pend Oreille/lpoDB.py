...
class LPO_DB:
    ...
    def __iter__(self):
        ...
        cursor = self.db.execute(
            "SELECT * FROM {} ORDER BY Date, Time".format(self.table)
        )
        for row in cursor:
            yield dict(row)
    ...
    def get_data_for_range(self, start, end):
        ...
        for year in range(start.year, 2007):
            if (
                list(self._get_status_for_range(date(year, 1, 12), date(year, 1, 12)))
                == []
            ):
        ...
        for entry in statuses:
            if entry["Status"] == "COMPLETE":
                dates_to_update.remove(
                    datetime.strptime(str(entry["Date"]), "%Y%m%d").date()
                )
            elif entry["Status"] == "PARTIAL":
                try:  # update for any new data first, then remove from dates_to_update
                    # list
                    self._update_data_for_date(
                        datetime.strptime(str(entry["Date"]), "%Y%m%d").date(), True
                    )
                except:
                    raise
                dates_to_update.remove(
        ...
        if error_dates != []:
            error_message = (
                "There were problems accessing data for the following "
                "dates. They were not included in the result.\n"
            )
            for day in error_dates:
                error_message += "\n{}".format(day)
            messagebox.showwarning(title="Warning", message=error_message)

        # get Air_Temp, Barometric_Press, and Wind_Speed data from specified start/end
        # date range
        cursor = self.db.execute(
            """SELECT Air_Temp, Barometric_Press, Wind_Speed
                                   FROM {} WHERE Date BETWEEN {} AND {}""".format(
                self.table, start.strftime("%Y%m%d"), end.strftime("%Y%m%d")
            )
        )
        for row in cursor:
            yield dict(row)
    ...
    def _get_status_for_range(self, start, end):
        ...
        cursor = self.db.execute(
            """SELECT DISTINCT Date, Status FROM {}
    ...
    def _update_data_for_date(self, date, partial):
        ...
        if partial:
            self.db.execute(
                "DELETE FROM {} WHERE Date={}".format(
                    self.table, date.strftime("%Y%m%d")
                )
            )
            self.db.commit()
        ...
        for entry in data:
            self.db.execute(
                """INSERT INTO {} (Date, Time, Status, Air_Temp, Barometric_Press, Wind_Speed)
                                VALUES (?, ?, ?, ?, ?, ?)""".format(
                    self.table
                ),
                (
...
def test():
    ...
    db = LPO_DB(filename="test.db", table="Test")
    db.clear()
    db.close()
    ...
    if dict(db) != {}:
        print("Error in LPO_DB test(): Database is not empty")

    # add data for current date
    try:
        db.update_data_for_date(date.teoday(), False)
    except:
        print("ERROR in LPO_DB.test(): Could not retrieve data for today\n")
