...
class Feedback:
    def __init__(self, master):
        ...
        ttk.Label(
            self.frame_header, text="Thanks for Exploring!", style="Header.TLabel"
        ).grid(row=0, column=1)
        ttk.Label(
            self.frame_header,
            wraplength=300,
            text=(
                "We're glad you chose Explore California for your recent adventure.  "
        ...
        ttk.Label(self.frame_content, text="Comments:").grid(
        ...
        self.text_comments = Text(
            self.frame_content, width=50, height=10, font=("Arial", 10)
        )
        ...
        ttk.Button(self.frame_content, text="Submit", command=self.submit).grid(
            row=4, column=0, padx=5, pady=5, sticky="e"
        )
        ttk.Button(self.frame_content, text="Clear", command=self.clear).grid(
    ...
    def submit(self):
        ...
        self.clear()
        messagebox.showinfo(
            title="Explore California Feedback", message="Comments Submitted!"
        )
