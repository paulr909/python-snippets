#!/usr/bin/python3
# captionsort.py by Barron Stone
...
class CaptionSort:
    def __init__(self, master):
        ...
        ttk.Label(self.main_frame, text="Source Directory:").grid(
        ...
        ttk.Button(
            self.main_frame, text="Browse...", command=self.browse_src_callback
        ).grid(row=1, column=1, sticky="w")

        ttk.Label(self.main_frame, text="Destination Directory:").grid(
        ...
        ttk.Button(
            self.main_frame, text="Browse...", command=self.browse_dest_callback
        ).grid(row=3, column=1, sticky="w")

        self.copy_var = IntVar()
        self.copy_var.set(1)
        ttk.Checkbutton(
            self.main_frame, text="Copy Files", variable=self.copy_var
        ).grid(row=4, column=0, columnspan=2)

        ttk.Button(
            self.main_frame, text="Sort Images", command=self.sort_callback
    ...
    def sort_callback(self):
        image_paths = []  # list of all image files in directory
            ...
            try:  # use ExifTool from http://www.sno.phy.queensu.ca/~phil/exiftool/
                ...
                if caption:
                    try:  # will fail if caption does not begin with a letter
                        ...
                        print(
                            "Error sorting {} - Caption begins with {}".format(
                                path, caption[0]
                            )
                        )
            ...
            if sort_dirs[key]:
                ...
                for file in sort_dirs[key]:
                    if self.copy_var.get():
                        try:
                            shutil.copy(
                                file, os.path.join(key_path, file.split("\\")[-1])
                            )
