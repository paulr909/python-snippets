...
class NQueens:
    def __init__(self, master):
        ...
        ttk.Label(
            self.controls_frame, text="Number of Queens:", font="Verdana 10 bold"
        ).grid(row=0, column=0)
        ...
        ttk.Button(
            self.controls_frame,
            text="Get Next Solution",
            command=self.solution_callback,
        ...
        self.time_var.set("--")
        ttk.Label(self.controls_frame, text="Solution:", font="Verdana 10 bold").grid(
            row=0, column=3, sticky=(E)
        )
        ttk.Label(
            self.controls_frame, textvariable=self.solution_var, font="Verdana 10"
        ).grid(row=0, column=4, sticky=(W))
        ttk.Label(
            self.controls_frame, text="Elapsed Time:", font="Verdana 10 bold"
        ).grid(row=1, column=3, sticky=(E))
        ttk.Label(
            self.controls_frame, textvariable=self.time_var, font="Verdana 10"
    ...
    def solution_callback(self):
        try:
            ...
            messagebox.showerror(
                title="Invalid Input", message="Must enter a number for N."
            )
            return
        ...
        if self.n != input_val or self.solutions == None:
            if 4 > input_val:
                messagebox.showerror(
                    title="Invalid Value for N", message="N must be greater than 4."
                )
            else:
                self.n = input_val
                self.index = 0
        ...
        self.solution_var.set(
            "{}/{}".format(self.index % len(self.solutions) + 1, len(self.solutions))
        )
        self.draw_board()
