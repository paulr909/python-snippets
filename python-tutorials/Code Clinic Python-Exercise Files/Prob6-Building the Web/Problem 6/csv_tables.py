...
CSV_FILES = (".\\_assets\\first_semester.csv", ".\\_assets\\second_semester.csv")
...
def create_row(row_data, **kwargs):
    ...
    if kwargs.get("is_header", False):
        ...
        output += (
            sp + "<td><strong>" + first_col[0] + " </strong>" + first_col[1] + "</td>\n"
        )
...
def create_table(table_data, **kwargs):  # create table from list-of-lists
    ...
    output += create_row(
        table_data[0], line_space=len(sp), is_header=True
...
def insert_csv(html_path, csv_paths):
    ...
    sp = len(html_halves[0]) - len(
        html_halves[0].rstrip(" ")
    )  # number of spaces before </article> tag
...
if __name__ == "__main__":
    insert_csv(HTML_PATH, CSV_FILES)
